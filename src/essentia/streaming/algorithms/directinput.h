...
#ifndef ESSENTIA_DIRECTINPUT_H
#define ESSENTIA_DIRECTINPUT_H
...
class DirectInput : public Algorithm {
 protected:
 ...
 public:
  ...
  DirectInput(const std::vector<TokenType>* input=0, bool own = false)
  ...
  DirectInput(std::vector<TokenType>* input, bool own = false)
    : _inputVector(input), _ownVector(own) {
    setName("DirectInput");
    declareOutput(_output, acquireSize, "data", "the values read from the vector");
    reset();
  ...
  DirectInput(const Array& inputArray, bool own = true) {
    setName("DirectInput");
  ...
  DirectInput(const TNT::Array2D<Real>& input) {
    setName("DirectInput");
  ...
  AlgorithmStatus process() {
    ...
    if (status != OK) {
      if (status == NO_OUTPUT) {
        throw EssentiaException("DirectInput: internal error: output buffer full");
      }
...
template <typename T>
void connect(DirectInput<T>& v, SinkBase& sink) {
...
template <typename T>
void operator>>(DirectInput<T>& v, SinkBase& sink) {
  connect(v, sink);
}
...
void connect(std::vector<T>& v, SinkBase& sink) {
  DirectInput<T>* vectorInput = new DirectInput<T>(&v);
